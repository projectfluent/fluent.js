...
suite("StringLiteral.parse()", function() {
  ...
  test("Unicode escapes", function() {
    ...
    assert.deepEqual(parseLiteral(`x = {"\\\\U01F602"}`), {value: "\\U01F602"});
  });
...
suite("NumberLiteral.parse()", function() {
  ...
  test("Padded floats", function () {
    assert.deepEqual(parseLiteral("x = {01.03}"), { value: 1.03, precision: 2 });
    assert.deepEqual(parseLiteral("x = {1.0300}"), { value: 1.03, precision: 4 });
    assert.deepEqual(parseLiteral("x = {01.0300}"), { value: 1.03, precision: 4 });
    assert.deepEqual(parseLiteral("x = {-01.03}"), { value: -1.03, precision: 2 });
    assert.deepEqual(parseLiteral("x = {-1.0300}"), { value: -1.03, precision: 4 });
    assert.deepEqual(parseLiteral("x = {-01.0300}"), { value: -1.03, precision: 4 });
  });
