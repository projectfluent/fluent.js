  ...
  switch (type) {
    case 'properties':
      return cache[res][lang] = io.load(url).then(function(source) {
        ...
        // Context._fetchResources (via `return Promise.reject(err)`) so that
        // Context.ready always fullfills.
...
Env.prototype._getEntity = function(lang, resIds, id) {
  ...
  for (var i = 0, resId; resId = resIds[i]; i++) {
    ...
    if (id in this._resCache[resId][lang]) {
      ...
      // return this._compiler.compile(this._resCache[resId][lang][id], lang);
