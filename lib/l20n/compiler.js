...
function Entity(id, node, lang) {
  ...
  // Entities; see Entity.prototype.formatValue
  this.dirty = false;
...
Entity.prototype.formatValue = function(ctx, args) {
  if (this.dirty) {
    throw new L10nError('Cyclic reference to ' + this.id);
  }
...
Entity.prototype.formatEntity = function(ctx, args) {
  if (!this.attributes) {
    return this.formatValue(ctx, args);
  }

  var entity = {
    value: this.formatValue(ctx, args),
    attributes: Object.create(null)
  };

  for (var key in this.attributes) {
    /* jshint -W089 */
    entity.attributes[key] = this.attributes[key].formatValue(ctx, args);
  }
...
function resolveIdentifier(ctx, lang, args, id) {
  ...
  return entity ? entity.formatValue(ctx, args) : undefined;
}
