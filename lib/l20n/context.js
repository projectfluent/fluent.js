...
function getWithFallback(id) {
  ...
  while (loc = this.supportedLocales[cur]) {
    ...
    if (entry === undefined) {
      cur++;
      reportMissing.call(this, new L10nError(
...
function formatValue(args, entity) {
  ...
  try {
    return Resolver.format(entity, args);
  } catch (err) {
...
function formatEntity(args, entity) {
  ...
  for (var key in entity.attrs) {
    /* jshint -W089 */
    formatted.attrs[key] = formatValue(args, entity.attrs[key]);
  }
