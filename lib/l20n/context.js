...
function Context(env, resIds) {
  ...
  this.ready = env.ready.then(function() {
    ...
    this._env._emitter.addEventListener(
      ...
      console.log('-- languagechange! ' + supported.join(', '));
...
Context.prototype._fetchResources = function(cur) {
  console.log('-- fetching resources for supported lang #' + cur);
...
Context.prototype._getFallback = function(cur, id, args) {
  ...
  if (!entity) {
    console.log('-- ' + id + ' missing from supported lang #' + cur);
    ...
    console.log('-- ' + id + ' found in supported lang #' + cur);
    // XXX format here, passing ctx and args
