...
function Context(env, resIds) {
  ...
  this.ready = env.ready.then(function(supported) {
    this._env.addEventListener('languagechange', function(supported) {
      debug('languagechange!', supported.join(', '));
      this.ready = this._fetchResources(supported[0]).then(function() {
        return supported;
      });
    }.bind(this));
