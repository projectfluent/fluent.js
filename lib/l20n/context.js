...
define(function (require, exports, module) {
  ...
  function Context(id) {
    ...
    _parser.addEventListener('error', err);
    ...
    function getFromLocale(cur, id, data, prevSource) {
      ...
      if (!loc) {
        err(new RuntimeError('Unable to get translation', id, _fallbackChain));
      ...
      try {
        ...
        if (e instanceof Compiler.RuntimeError) {
          err(new TranslationError(e.message, id, _fallbackChain, locale));
    ...
    function requestLocales() {
      ...
      .then(setReady)
      .then(null, function handleError(e) {
        throw err(e);
      });
