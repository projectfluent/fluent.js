...
define(function (require, exports, module) {
  ...
  var IO = require('./bindings/html/io').IO;
  ...
  function Resource(id, parser) {
    ...
    function fetch(async) {
      if (!async) {
        if (!self.source) {
          self.source = IO.loadSync(self.id);
        }
        return;
      }
      if (self.source) {
  ...
  function Context(id) {
    ...
    function linkTemplate(uriTemplate) {
      ...
      for (var lang in _locales) {
        uri = uriTemplate(lang);
        inRes = _locales[lang].resources.some(function(res) {
          return res.id === uri;
        });
        if (!inRes) {
    ...
    function linkURI(uri) {
      ...
      if (_none === undefined) {
        ...
        inRes = _none.resources.some(function(res) {
          return res.id === uri;
        });
      }
      if (!inRes) {
