...
(function() {
  ...
  function Compiler(Emitter, Parser) {
    ...
    Entity.prototype.toString = function toString(ctxdata) {
      try {
        ...
        // created.  This is because the underlying error will have been 
        // already emitted as a `RuntimeError`.  To avoid duplication, the 
        // `ValueErrors` will only be emitted if it actually made it to here.  
        // See `HashLiteral` for an example of why it wouldn't make it.
    ...
    function Identifier(node, entry) {
      ...
      return function identifier(locals, ctxdata) {
        if (!_env.hasOwnProperty(name)) {
          throw emit(RuntimeError, 'Reference to an unknown entry: ' + name,
                     entry);
        }
    ...
    function VariableExpression(node, entry) {
      ...
      return function variableExpression(locals, ctxdata) {
        ...
        if (!ctxdata || !ctxdata.hasOwnProperty(name)) {
          throw emit(RuntimeError, 'Reference to an unknown variable: ' + name,
                     entry);
        }
    ...
    function GlobalsExpression(node, entry) {
      ...
      return function globalsExpression(locals, ctxdata) {
        if (!_globals) {
          throw emit(RuntimeError, 'Globals missing (tried @' + name + ').',
                     entry);
        }
        if (!_globals.hasOwnProperty(name)) {
          throw emit(RuntimeError, 'Reference to an unknown global: ' + name,
                     entry);
        }
    ...
    function ArrayLiteral(node, entry, index) {
      var content = [];
      var defaultIndex = index ? index.shift() : null;
    ...
    function ComplexString(node, entry) {
      ...
      return function() {
        ...
        return function complexString(locals, ctxdata) {
          if (dirty) {
            throw emit(RuntimeError, "Cyclic reference detected", entry);
          }
          dirty = true;
          var parts = [];
          try {
    ...
    function BinaryOperator(token, entry) {
      ...
      if (token == '/') return function devideOperator(left, right) {
        ...
        if (right == 0) {
          throw emit(RuntimeError, 'Division by zero not allowed.', entry);
        }
        return left / right;
      };
    ...
    function CallExpression(node, entry) {
      ...
      return function callExpression(locals, ctxdata) {
        ...
        if (!macro._call) {
          throw emit(RuntimeError, 'Expected a macro, got a non-callable.',
                     entry);
        }
      ...
      return function propertyExpression(locals, ctxdata) {
        ...
        if (typeof parent !== 'function') {
          if (!parent.hasOwnProperty(prop)) {
            throw emit(RuntimeError,
                        prop +' is not defined in the context data');
          }
  ...
  function delegate(delegator, delegatee) {
    var Constructor = function() {
      delegatee.apply(this, arguments);
      delegator.apply(this, arguments);
    }
    Constructor.prototype = Object.create(delegatee.prototype);
    Constructor.prototype.constructor = delegator;
    return Constructor;
  }

  // `CompilerError` is a general class of errors emitted by the Compiler.
  var CompilerError = delegate(function(message, entry) {
  ...
  var CompilationError = delegate(function(message, entry) {
  ...
  // `entity.toString()`.
  var RuntimeError = delegate(function(message, entry) {
  ...
  var ValueError = delegate(function(message, entry, source) {
  ...
  var IndexError = delegate(function(message, entry) {
