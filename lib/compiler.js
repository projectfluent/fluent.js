(function() {
  ...
  function _resolve(expr, locals, env, data, index) {
    ...
    if (expr instanceof Entity) {
      return expr.toString(env, data, index);
    }
    if (expr instanceof Attribute) {
      return expr.toString(locals, env, data, index);
    }
    var index = index || [];
    var key = index.shift();
  ...
  function _yield(expr, locals, env, data, key) {
    ...
    if (current instanceof Entity) {
      return current._yield(env, data, key);
    }
    if (current instanceof Attribute) {
      return current._yield(locals, env, data, key);
    }
  ...
  function PropertyExpression(node) {
    ...
    return function propertyExpression(locals, env, data) {
      property = _resolve(property, locals, env, data);
      return _yield(expression, locals, env, data, property);
    }
  }
  function AttributeExpression(node) {
    ...
    return function attributeExpression(locals, env, data) {
      attribute = _resolve(attribute, locals, env, data);
  ...
  Attribute.prototype.toString = function toString(locals, env, data, index) {
    var index = index || locals.__this__.index;
    return _resolve(this.value, locals, env, data, index);
  };
  ...
  Entity.prototype.toString = function toString(env, data, index) {
