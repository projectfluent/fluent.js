(function() {
  ...
  function _step(expr, locals, env, data, key) {
  ...
  function _resolve(expr, locals, env, data, index) {
    ...
    var index = index || [];
    var key = index.shift();
    var current = _step(expr, locals, env, data, key);
    return _resolve(current.self, current.locals, env, data, index);
  }
  ...
  function CallExpression(node) {
    ...
    return function callExpression(locals, env, data) {
      ...
      var macro = _step(callee, locals, env, data);
      // XXX what if it's not a macro?
      var resolved_args = [];
      args.forEach(function(arg, i) {
        resolved_args.push(_resolve(arg, locals, env, data));
      });
      // rely entirely on the platform implementation to detect recursion
      return macro.self(macro.locals, env, data, resolved_args);
    };
  }
  function PropertyExpression(node) {
    var expression = new Expression(node.expression);
    var property = node.computed ? 
      new Expression(node.property) : 
      node.property.name;
    return function propertyExpression(locals, env, data) {
      var parent = _step(expression, locals, env, data);
      var prop = _resolve(property, locals, env, data);
      if (parent.self._yield) {
        // it's an Entity or an Attribute
        return parent.self._yield(data, prop);
      }
      return parent.self(parent.locals, env, data, prop);
    }
  }
  function AttributeExpression(node) {
    var expression = new Expression(node.expression);
    var attribute = node.computed ? 
      new Expression(node.attribute) : 
      node.attribute.name;
    return function attributeExpression(locals, env, data) {
      var entity = _step(expression, locals, env, data);
      // XXX what if it's not an entity?
      var attr = _resolve(attribute, locals, env, data);
      return [entity.locals, entity.self.attributes[attr]];
    }
  }
