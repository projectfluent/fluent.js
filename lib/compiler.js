    ...
    function HashLiteral(node, entry, index) {
      ...
      return function hashLiteral(locals, ctxdata, prop) {
        ...
        for (var i = 0; i < keysToTry.length; i++) {
          ...
          // if the `prop` was not passed, the `key` is `undefined`;  if there 
          // is no default index nor default (catch-all) key; the `key` is 
          // `null`
          if (key === undefined || key === null) {
            continue;
          }
          if (typeof key !== 'string' || key.length === 0) {
            throw emit(IndexError, 'Index must be a non-empty string', entry);
          }
          keysTried.push(key);
