...
// more logic to build() than parse/fetch etc.
  ...
  function Resource(id, parser) {
    ...
    function build(nesting) {
      ...
      return fetch().then(parse).then(function() {
  ...
  function Locale(id, parser, compiler) {
    ...
    function freeze(callback) {
      var self = this;
      this.resource.build(0).then(function freeze_success() {
  ...
  function Context(id) {
    ...
    function get(id, data) {
      var entry = _locales[0].getEntry(id);
      if (entry !== undefined) {
        try {
        ...
        return fallback(new ContextError('missing'));
      }
    }

    function fallback(error) {
      if (error instanceof ContextError) {
        _locales[1].freeze(null, true); // sync option
        return _locales[1].getEntry(id).toString();

        console.log('missing');
      }
      if (error instanceof _compiler.ValueError) {
        console.log(error.source);
      }
      if (error instanceof _compiler.IndexError) {
