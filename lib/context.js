...
// more logic to build() than parse/fetch etc.
  ...
  function Context(id) {
    ...
    function freeze() {
      _isFrozen = true;
      _languages = L20n.Intl.prioritizeLocales(_available);
      // compile the first locale on the fallback list
      var locale = _available.length > 0 ? _locales[_languages[0]] : _none;
