(function() {
  ...
  function Locale(id, parser, compiler) {
    ...
    var _resource = new Resource(null, parser);
    var _entries = null;

    function addResource(text) {
      _resource.addResource(null, text);
    }

    function linkResource(text) {
      _resource.addResource(null, text);
    }
    ...
    function freeze(callback) {
      _resource.build().then(
        function freeze_success() {
          _entries = compiler.compile(_resource.ast);
          callback();
        }
      );
    }
  ...
  function Context(id) {
    ...
    function get(id, data) {
      return _locales[0].get(id, getArgs(data));
    }
