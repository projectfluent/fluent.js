...
// more logic to build() than parse/fetch etc.
  ...
  function Context(id) {
    ...
    function get(id, data, callback) {
      return getFallback(id, data, callback, false);
    }

    function getEntity(id, data, callback) {
      return getFallback(id, data, callback, true);
    }
    ...
    function getFallback(id, data, callback, returnEntity) {
      if (!_isReady) {
        if (!callback) {
      ...
      while (locale = getLocale(i)) {
        ...
        try {
          var method = returnEntity ? 'get' : 'toString';
          var val = entry[method](getArgs.bind(this, data));
          if (i > 0) {
    ...
    function getPending() {
      while (_pending.length) {
        ...
        return getFallback.apply(this, req);
      }
    }
    ...
    function freeze() {
      ...
      return locale.build(true).then(onCompile);
    }
