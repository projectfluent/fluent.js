(function() {
  ...
  Cache.prototype.get= function Cache_get(id) {
    if (!this.items[id]) {
      this.items[id] = new this.ctor(id, this.ctx);
    }
  ...
  function ProcessedResource(id, ctx) {
    ...
    function importFirstURL(urls, callback, fallback, nesting, async) {
      var url = urls.shift();
      var imported = self.ctx._cache.get(url);
  ...
  function Context() {
    ...
    // context
    this._cache = new Cache(this, ProcessedResource);
    ...
    function createSubContext(async) {
      ...
      debug('subcontext\'s locale is ', subctx.settings.locales[0])
    ...
    function invalidateOnIntegrityError() {
      isIntegral = false;
      meta.abortImports();
      debug('invalidated context\'s locale was ', self.settings.locales[0]);
