(function() {
  ...
  var Cache = {
    ...
    getResource: function(relpath, basepath) {
    ...
    normalizeURL: function(relpath, basepath) {
      var parts;
      if (relpath[0] == '/') {
        parts = relpath.split('/');
      } else if (basepath) {
  ...
  function Resource(uri) {
    ...
    this.lol = uri ? Cache.getLOLFile(uri) : null;
    ...
    this.loadResource = function loadResource(relpath, callback, nesting) {
      ...
      var imported = Cache.getResource(relpath, self.dirname);
  ...
  function Context() {
    ...
    this.addResource = function(uri) {
      ...
      meta.loadResource(uri, _ifComplete(meta, compile));
    };
