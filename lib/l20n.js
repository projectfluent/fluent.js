(function() {
  ...
  function Context(id) {
    ...
    var _settings = {
      'schemes': ['./locales/{{locale}}/{{resource}}.lol'],
      'locales': ['pl', 'en-US'],
    };
    ...
    function loadResource(res, sync) {
      if (!sync) {
        var deferred = when.defer();
      }
      var uri = resolveURI(res.id, _settings.locales[0], _settings.schemes);
      if (sync) {
        res.source = downloadSync(uri[0]);
        onResourceLoaded(res);
      } else {
        downloadAsync(uri[0]).then(
