(function() {
  ...
  function ProcessedResource(cache, uri) {
    ...
    this.import = function pr_import(relpath, callback, nesting) {
      nesting = nesting || 0;
      if (nesting > 7) {
  ...
  function Context() {
    ...
    function resolveUrl(url) {
      if (/^l10n:/.test(url)) {
        if (settings.locales === null ||
            settings.paths === null) {
          throw "Can't use schema urls without settings.locales|paths";
        }
    ...
    this.addResource = function ctx_addResource(url) {
      ...
      meta.import(resolveUrl(url), _ifComplete(meta, compile));
    };
