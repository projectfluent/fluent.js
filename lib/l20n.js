(function() {
  ...
  L20n.XHR_ERROR = 4;
  L20n.PARSE_ERROR = 8;
  L20n.CORRUPTED_ERROR = 16;

  // Preprocessing errors
  L20n.NESTED_ERROR = 32;
  L20n.RESOURCENOTFOUND_ERROR = 64;
  L20n.PREPROCESSING_ERROR = 128;

  // Context errors
  L20n.INTEGRITY_ERROR = 256;
  L20n.COMPILE_ERROR = 512;
  L20n.NOMORELOCALES_ERROR = 1024;
  ...
  function Resource(id) {
    ...
    function downloadAsync(url, callback, fallback) {
      ...
      xhr.addEventListener('load', function() {
        if (xhr.status == 200) {
          ...
          isCorrupted = true;
          var exception = new Exception(L20n.HTTP_ERROR,
      ...
      xhr.addEventListener('error', function() {
        ...
        var exception = new Exception(L20n.XHR_ERROR,
    ...
    this.get = function r_get(callback, fallback, async) {
      if (ast) {
        ...
        var exception = new Exception(L20n.CORRUPTED_ERROR,
                                      'Resource "' + id + '" is corrupted');
        fallback(exception);
      } else {
  ...
  function ProcessedResource(id, ctx, cache) {
    ...
    this.load = function pr_load(callback, fallback, nesting, async) {
      if (this.isReady) {
        ...
        var exception = new Exception(L20n.PREPROCESSING_ERROR,
                                      'ProcessedResource "' + id + '" is corrupted');
        fallback(exception);
      } else {
        callbacks.push(callback);
    ...
    function importFirstURL(urls, callback, fallback, nesting, async) {
      ...
      imported.load(
        ...
        function importFailed(ex) {
          debug('calling fallback,', url, ' failed to load');
          var preprocessingErrors = L20n.NESTED_ERROR |
                                    L20n.RESOURCENOTFOUND_ERROR |
                                    L20n.PREPROCESSING_ERROR;
          if (ex.code & preprocessingErrors) {
            fallback(ex);
          } else if (urls.length == 0) {
            debug('no more scheme URLs to try');
            var exception = new Exception(L20n.RESOURCENOTFOUND_ERROR);
            fallback(exception);
          } else {
    ...
    this.importResource = function pr_importResource(uri, 
      ...
      nesting = nesting || 0;
      if (nesting > 7) {
        var exception = new Exception(L20n.NESTED_ERROR);
        fallback(exception);
        return false;
      }
  ...
  function Context() {
    ...
    function handleExceptions(ex) {
      var preprocessingErrors = L20n.NESTED_ERROR |
                                ...
                                L20n.PREPROCESSING_ERROR;
      if (ex.code & preprocessingErrors) {
        ex.code |= L20n.INTEGRITY_ERROR;
        emitter.emit('error', ex);
        invalidateOnIntegrityError();
      }
      return false;
    }
