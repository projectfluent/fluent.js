(function() {
  ...
  L20n.INTEGRITY_ERROR = 2;
  L20n.NESTED_ERROR = 4;
  L20n.MISSING_ERROR = 8;
  L20n.TIMEOUT_ERROR = 16;
  L20n.NOMOREPATHS_ERROR = 32;
  L20n.NOMORELOCALES_ERROR = 64;
  L20n.CORRUPTED_ERROR = 128;
    ...
    function downloadAsync(url, callback, fallback) {
      ...
      xhr.addEventListener('load', function() {
        if (xhr.status == 200) {
          ...
          isCorrupted = true;
          var exception = new Exception(L20n.MISSING_ERROR,
    ...
    function importFirstURL(urls, callback, fallback, nesting, async) {
      ...
      imported.load(
        ...
        function importFailed(ex) {
          ...
          if (ex.code == L20n.NESTED_ERROR) {
            fallback(ex);
            return false;
          } else if (ex.code == L20n.NOMOREPATHS_ERROR || urls.length == 0) {
            debug('no more scheme URLs to try, creating a subcontext');
            // fallback to the subctx and bubble the exception
            if (urls.length == 0) {
              var exception = new Exception(L20n.NOMOREPATHS_ERROR);
              fallback(exception);
            } else {
              var exception = new Exception(L20n.INTEGRITY_ERROR);
              fallback(exception);
            }
          } else {
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      meta.importResource(uri, _ifComplete(meta, compile), 
                          function(ex) {
                            ...
                            if (ex.code == L20n.NOMOREPATHS_ERROR) {
