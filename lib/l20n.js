  ...
  function Resource(id) {
    ...
    function downloadAsync(url, callback, fallback) {
      ...
      xhr.addEventListener('load', function() {
        if (xhr.status == 200) {
          ...
          exception.code = errorCodes['missing'];
          exception.text = 'XHR returned an error code for url: '+url;
          exception.data = xhr.status;
          fallback(exception);
        }
      });
      ...
      xhr.addEventListener('error', function() {
        ...
        var exception = new Exception();
        exception.code = errorCodes['missing'];
        exception.text = 'XHR returned an error for url: '+url;
        fallback(exception);
      });
    ...
    this.get = function r_get(callback, fallback, async) {
      if (ast) {
        ...
        var exception = new Exception()
        exception.code = errorCodes['corrupted'];
        exception.text = 'Resource "'+id+'" is corrupted';
        fallback(exception);
      } else {
  ...
  function ProcessedResource(id, ctx, cache) {
    ...
    this.load = function pr_load(callback, fallback, nesting, async) {
      if (this.isReady) {
        ...
        var exception = new Exception();
        exception.code = errorCodes['corrupted'];
        exception.text = 'ProcessedResource "'+id+'" is corrupted';
        fallback(exception);
      } else {
        callbacks.push(callback);
    ...
    function importFirstURL(urls, callback, fallback, nesting, async) {
      ...
      imported.load(
        ...
        function importFailed(ex) {
          ...
          if (ex.code == errorCodes['nested']) {
            fallback(ex);
            return false;
          } else if (ex.code == errorCodes['nomorepaths'] || urls.length == 0) {
            debug('no more scheme URLs to try, creating a subcontext');
            // fallback to the subctx; true bubbles the integrityError up to 
            // the parent fallback
            var exception = new Exception();
            exception.code = errorCodes['nomorepaths'];
            fallback(exception);
          } else {
    ...
    this.importResource = function pr_importResource(uri, 
      ...
      if (nesting > 7) {
        var exception = new Exception();
        exception.code = errorCodes['nested'];
        fallback(exception);
        return false;
      }
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      meta.importResource(uri, _ifComplete(meta, compile), 
                          function(ex) {
                            ...
                            if (ex.code == errorCodes['nomorepaths']) {
