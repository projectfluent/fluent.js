(function() {
  ...
  function ProcessedResource(id, ctx) {
          ...
          if (imports.length) {
            ...
            imports.forEach(function(node){
              ...
              self.importResource(node.uri.content, 
                _ifComplete(self, preprocess), nesting + 1, async);
            });
          } else {
            self.ast = rawast;
            self.isReady = true;
    ...
    function importFirstURL(urls, callback, nesting, async) {
      ...
      imported.load(
        ...
        function importFailed() {
          ...
          if (urls.length) {
            ...
            var pos = self.imports.indexOf(imported);
            if (pos > -1) {
              self.imports.splice(pos, 1);
            }
            importFirstURL(urls, callback, nesting, async);
          } else {
    ...
    this.importResource = function pr_importResource(uri, callback, nesting, async) {
      nesting = nesting || 0;
      if (nesting > 7) {
  ...
  function Context() {
    ...
    function getSync(id, data) {
      ...
      if (!entity) {
        debug('entity', id, 'not found, creating subcontext');
        subctx = createSubContext();
        debug(subctx)
        entity = subctx.get(id, data);
      }
    ...
    this.reset = function ctx_reset() {
      ...
      uris.forEach(function(uri) {
        meta.importResource(uri, _ifComplete(meta, compile), 0, true);
      });
    };
    ...
    this.addResource = function ctx_addResource(uri, async) {
      ...
      uris.push(uri);
      meta.importResource(uri, _ifComplete(meta, compile), 0, async);
    };
