...
var IO = require('./io.js').IO;
  ...
  var L20n = {
    ...
    get parser () {
      if (!this._parser) {
        this._parser = new L20n.Parser();
      }
      return this._parser;
    }
  };

  function SourceResource(url, text) {
    var self = this;
    ...
    function parse() { 
      self.ast = L20n.parser.parse(self.text);
    }
  }
  ...
  function Resource(id, ctx) {
          ...
          if (node.type == 'ImportStatement') {
            ...
            if (self.resources.length) {
              Array.prototype.splice.apply(self.ast.body,
                                          [i, 1].concat(self.resources.shift().source.ast.body));
            }
          } 
        }
      }
    ...
    function parse(nesting) {
      this.source.parse();
      var imports = this.source.ast.body.filter(function(elem) {
        return elem.type == 'ImportStatement';
      });
      imports.forEach(function(imp) {
        addResource(imp.uri.content, true, nesting + 1); 
      });
    }

    function injectResource(id, text) {
      var res = new Resource(id, this);
      var source = new SourceResource(id, text);
      res.source = source;
      source.text = text;
      source.parse();
      res.isReady = true;
    ...
    function loadSourceAsync(uri) {
      var deferred = when.defer();
      var url = resolveURI(uri, ctx.settings.locales[0], ctx.settings.schemes[0]);
      if (self.source) {
      ...
      IO.loadAsync(url).then(
        function loadResource_success(text) {
          var source = new SourceResource(url, text);
    ...
    function loadSourceSync(uri, attempt) {
      ...
      var text = null;
      while (ctx.settings.schemes.length >= attempt+1) {
    ...
    function addResource(uri, async, nesting) {
      ...
      var res = new Resource(uri, ctx);
      self.resources.push(res);
      if (async) {
        loadSourceAsync(uri).then(
          function(source) {
            if (source) {
              res.source = source;
              res.parse(nesting);
  ...
  function Context(id) {
    ...
    var mData = {};
    var _isFrozen = false;
    var _resource = new Resource(null, this);
    var _entries = {};
    var _ctxdata = {};
    var _globals = {};
