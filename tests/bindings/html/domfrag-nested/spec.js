describe('First translation', function() {
  ...
  it('KB should be bold', function() {
    var node = document.querySelector('#progress1 abbr');
    var css = window.getComputedStyle(node);
    expect(css.getPropertyValue('font-weight')).toEqual('700');
  });
...
describe('Retranslation', function() {
  ...
  it('KB should still be bold', function() {
    var node = document.querySelector('#progress1 abbr');
    var css = window.getComputedStyle(node);
    expect(css.getPropertyValue('font-weight')).toEqual('700');
  });
