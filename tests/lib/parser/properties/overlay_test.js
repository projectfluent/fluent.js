...
describe('HTML detection', function() {
  ...
  it('in simple values', function() {
    var ast = PropertiesParser.parse(null, 'id = string <a>');
    assert.deepEqual(ast[0].$v, { $o: 'string <a>' });
  });

  it('in complex values', function() {
    var ast = PropertiesParser.parse(null, 'id = string <a> {{ b }}');
  ...
  it('in simple attributes', function() {
    var ast = PropertiesParser.parse(null, 'id.attr = string <a>');
